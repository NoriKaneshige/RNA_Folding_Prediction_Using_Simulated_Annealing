...
def visualize_structure(bp_matrix, label = 'UUCG-tetraloop'):
  # initialise figure
...
def setup_stems(bp_compatiable_matrix):
    ...
    # print("stem_indices: {}".format(stem_indices))
    # print("stem_compatibility_matrix: {}".format(stem_compatibility_matrix))
    # print("crossing_matrix: {}".format(crossing_matrix))
    ...
    for i in range(0, bp_compatiable_matrix.shape[0]):
        ...
        for j in js:
            if j > i:
                ...
                stems_s1.append(s1)
                stems_s2.append(s2)
    # print("stems_s1: {}".format(stems_s1))
    # print("stems_s2: {}".format(stems_s2))
...
def trace_stem(bp_compatiable_matrix, i, j):
    ...
    while not terminal:
        if bp_compatiable_matrix[i,j] != 0:         
            ...
            terminal = True
    # print("stem_s1: {}".format(stem_s1))
    # print("stem_s2: {}".format(stem_s2))
...
def get_stem_energies(stems_s1, stems_s2, HB_matrix, G_HB = -1.89, G_stack = -4.5):
    ...
    for index_a in range(0, len(stems_s1)):
        ...
        # appending all energies of all stems in a state
...
def initialize_RNA(sequence = 'GGCACUUCGGUGCC', G_HB = -1.89, G_stack = -1.0):
    """ initialize dictionary that stores all information needed about an RNA,
        and which are derived from the sequence
    """
    # creating a dictionary called "state"
    state = {}        
    # taking an element one by one from 'GGCACUUCGGUGCC' and put each into a list called "sequence"
    sequence = [i for i in sequence]
    # numbering residues, starting from 0, ending with 13
    residues = [ i for i in range(0, len(sequence))]
    # generate all the possible base-pair combinations and if a base-pair is a normal combination, it puts 1.0
    # Also look at the type of base-pair and puts hydrogen bond numbers into a matrix
    bp_compatiable_matrix, hb_matrix = generate_basepair_compatibility_matrix(sequence)
    # call setup_stems function, which takes bp_compatible_matrix
    ...
    # appending all energies of all stems in a state to a list called "stem_energies"
    state['stem_energies'] = stem_energies
    state['assembled_stems'] = assembled_stems

    # the output is a dictionary called "state" 
    return(state)
...
def perturb_stem(state):    
        ...
        while not modified:
            ...
            # either remove from current structure
            # or add stem, if it compatible with stems in current structure
            if trial in assembled_stems_tmp:
